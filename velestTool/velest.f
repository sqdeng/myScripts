...
c
      ...
      write(headerline(1),'(1x,''>>> Start of program VELEST at '',a24,
     &              '' <<<'')') ctime
      write(6,'(1x,''>>> Start of program VELEST at '',a24,
     &          '' <<<'')') ctime
      write(6,*)
c
      write(headerline(2),*)'::::::::::::::::::::::::::::::::::::::'
...
c
      if(irayout.eq.1) open(13,file=rayfile,status='unknown')
      if(idrvout.eq.1) open(21,file=drvfile,status='unknown')
c
...
c
      if(icnvout.eq.1) open(7, file=cnvfile,status='unknown')
      if(ismpout.eq.1) open(11,file=smpfile,status='unknown')
      if(isingle.ne.0) open(2, file=velfile,status='unknown')  !  statisL-compatible output 
      if(ialeout.eq.1) open(75,file=alefile,status='unknown')  !  x,y, ALE of all events
      if(idspout.eq.1) open(76,file=dsprfile,status='unknown')  !  x,y, DSPR of all events
      if(irflout.eq.1) open(77,file=rflfile,status='unknown')  !  x,y, resid. of reflected ray
      if(irfrout.eq.1) open(78,file=rfrfile,status='unknown')  !  x,y, resid. of refracted ray
      if(iresout.eq.1) open(79,file=resfile,status='unknown')  !  dist, residual of ray
C 		rmsfile='velout.rms'
 		write(*,*)'the rms file is: ',rmsfile
		open(98,file=rmsfile,status='unknown')  !  rms of every iteration 
      k=0
...
cek
		if(ismpout.eq.1.and.isingle.eq.1) close(11)
         ...
         write(16,'(x,''>>>   End of program VELEST at '',a24,
     &           '' <<<'','' [ CPU-sec : '',f7.1,'']'')') ctime,cpusec
         endif
         write(6,'(x,''>>>   End of program VELEST at '',a24,
     &           '' <<<'','' [ CPU-sec : '',f7.1,'']'')') ctime,cpusec
...
c     do scaling of damping-factors (and determine ICOUNT):
     ...
     &              write(16,*)'~~~ set units for first iteration ...'
	  write(*,*)'~~~ set units for first iteration ...'
...
c
     ...
     &'' depth       x         y         z    mag ifx OBStot
     &   obsP obsS'')')
         else
         write(16,'(1x,'' eq    origin-time       latitude longitude '',
     &'' depth       x         y         z    mag ifx OBS'')')
         endif
         endif
      do i=1,legs
         avres(i)=0.
...
c
     ...
     &                       '' --> hypocenter and model backup.'')')
		...
		write(*,*)'~~~ solution is worse --> backup ...',ibackups
               endif
               call BACKUP
               ibackups=ibackups+1
               call STEPLENGTHCALC
         ...
         if(.not.single_turbo)then
            write(16,*)'50 times backup made!'
         endif
      endif
c
c     come here, if no backup made or 50 backups made
c
      if(.not.single_turbo)then
...
c
      if(ibackups.eq.50)then
         if(.not.single_turbo)then
            write(16,*)'50 TIMES BACKUP MADE --> sTOP ITERATIONS.'
         endif
         goto 9999
      endif
c
...
c
      if(isingle.eq.0)then
         ...
         write(16,'(x,''>>>   End of program VELEST at '',a24,
     &           '' <<<'','' [ CPU-sec : '',f7.1,'']'')') ctime,cpusec
         write(6,'(1x,''>>>   End of program VELEST at '',a24,
     &           '' <<<'','' [ CPU-sec : '',f7.1,'']'')') ctime,cpusec
      endif
c
c
...
c     Open control-inputfile:		
        ...
        if(narguments.lt.1) then
           write(*,'(/,a)') 'PARAMETER control FILE [velest.cmn <ret>]:'
           read(5,'(a)') parafile
           if(LEN_TRIM(parafile).le.1) then
              parafile= 'velest.cmn' !default control file name
           else
              parafile= parafile(1:LEN_TRIM(parafile))
           endif
        else
           call getarg(1,parafile)
        endif
		
      inquire(file=parafile,exist=lexist)
      if(.not.lexist)then
         stop'INPUTPARAM>>> control file not found!'
      endif
cVMS      open(10,file='velest.cmn',status='old',err=9910,readonly)
      open(10,file=parafile,status='unknown',err=9910)
...
c
      if(i.eq.34)then
         ...
         read(line(33),'(a)') rmsfile
         if(rmsfile.eq.' ') rmsfile='velout.rms'
         read(line(34),'(a)') outmodfile
         if(outmodfile.eq.' ') outmodfile='velout.mod'
      endif
c
      single_turbo=.false.
...
cVMS         open(16,file=outfile,status='new',carriagecontrol='list')
     ...
     &              ''EARTHQUAKES for simult. inversion IEQ = '',i8)')
     &              ieq
         write(16,'('' Max. number of '',
     &              ''SHOTS for simult. inversion    INSHOT = '',i8)')
     &              inshot
         write(16,'('' Max. number of '',
     &              ''(different) 1D-MODELS      ITOTMODELS = '',i8)')
     &              itotmodels
         write(16,'('' Max. number of '',
     &              ''LAYERS per one 1D-model        INLTOT = '',i8)')
     &              inltot
         write(16,'('' Max. number of '',
     &              ''STATIONS in stationlist           IST = '',i8)')
     &              ist
         write(16,'('' Max. number of '',
     &              ''OBSERVATIONS per event MAXOBSPEREVENT = '',i8)')
     &              maxobsperevent
         write(16,*)
         write(16,*)
      endif
...
c
     ...
     &              'zshift   itrial ztrial    ised'
      write(16,'(1x,f10.4,2x,f11.4,6x,i1,10x,f7.3,7x,i1,3x,f6.2,6x,i1)')
...
11       format(1h ,'layer    vel   depth   vdamp  reflector')
      ...
      do 9 j=1,nplay(i)
       if(j.eq.1)then
         read(10,*) vp(i,j),hp(i,j),vdamp(i,j)
...
c      if(reflch.ne.' ')then
c         if(reflch.eq.'m'.or.reflch.eq.'M')then
c            reflchar=reflch
c            ireflector=j
c            if(.not.single_turbo)then
c               if(vp(i,ireflector).gt.8.0)then
c                  write(16,*)'WARNING ::: velocity ABOVE reflector is',
c     &                       ' greater than 8.0 km/s  .....'
c                  write(6,*)'WARNING ::: velocity ABOVE reflector is',
c     &                       ' greater than 8.0 km/s  .....'
c                  write(6,*)
c               endif
c            endif
...
c            if(.not.single_turbo)then
c              write(16,*)'WARNING:'
c              write(16,*)'Reflector indicated in velocity-model is',
c     &                   ' marked with a : ',reflch
c              write(16,*)'Only   m   or   M   are allowed!'
c              write(16,*)'Reflected phases will be ignored (wrong mark)'
...
1     format(a80)
      if(.not.single_turbo)then
         write(16,*)
         write(16,2)
2        format(1h ,4x,' stn     latitude longitude  elev',
     &   8x,'x         y         z',5x,'ptcor stcor model icc')
      endif
      nsta=0

10    nsta=nsta+1
      read(10,fm,end=41) stn(nsta),xla(nsta),cns,xlo(nsta),cew,
     &            ielev(nsta),mode,icc,
...
27    format(1x,i4,1x,a7,f8.4,a1,f8.4,a1,1x,i5,2x,2f10.2,
     & f10.4,1x,2f6.2,2i4)
         if(cns.eq.'S') xla(nsta)=-xla(nsta)
         if(cew.eq.'E') xlo(nsta)=-xlo(nsta)
      endif
...
c
     ...
     &            eventtype, cns,cew
      character*7 sta(maxobsperevent)
...
c     input phase list and set-up initial trial hypocenter
      iunit=8
      if(i.eq.1)then
        if(neqs.gt.0) open(iunit,file=phasefile,status='old',err=998)                     
        if(nshot.gt.0) open(9,file=shotfile,status='old',err=999)                    
      endif
cek
...
cek print P and S number of obs
           if(nsp.eq.2) then
            write(16,'(1x,i5,1x,i4,2i2.2,1x,2i2.2,1x,f5.2,1x,f8.4,a1,
     &               1x,f8.4,a1,1x,f6.2,3f10.2,f5.2,i2,i4,3x,2i5)')
     ...
     &      emag(i),ifx(i),knobs(i)-nobswithw0,nobsp,nobss
            ...
            write(16,'(1x,i5,1x,i4,2i2.2,1x,2i2.2,1x,f5.2,1x,f8.4,a1,
     &                 1x,f8.4,a1,1x,f6.2,3f10.2,f5.2,i2,i4)')
     &      i,iyr(i),imo(i),iday(i),ihr(i),imin(i),e(1,i),
     &      xxlat,cns,aalon,cew,depth,xxx,(e(j,i),j=3,4),
     &      emag(i),ifx(i),knobs(i)-nobswithw0
         ...
         if(i.gt.neqs)then  !shots
            write(16,'(1x,i5,1x,i4,2i2.2,1x,2i2.2,1x,f5.2,1x,f7.4,a1,
...
c 1607 format(1x,i3,1x,i4,2i2.2,1x,2i2.2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,1x,
...
c
     ...
     &                       +stnazires(i,8)*res4) / tot
         if(tot.gt.0.0)then
            ...
            write(16,'(1x,i4,3x,a7,1x,5(f7.2,''('',i4,'')''))')
     &      i,stn(i),res0,ntot,res1,i1,
     &      res2,i2,res3,i3,res4,i4
         else
            write(16,'(1x,i3,3x,a7,1x,''-.-'')') i,stn(i)
         endif
      enddo
      write(16,*)
      write(16,*)
...
c
      write(2,'(1x,i4,2i2.2,1x,i2,'':'',i2,'':'',f6.3,1x
...
c
      ...
      aavdz=0.0
      write(16,*)
      write(16,'(4h  eq, 9x, 2hot, 9x, 1hx, 9x, 1hy, 9x, 1hz, 9x,3hrms,
     &6x,5havres,''          dot      dx       dy       dz'' )')
c
      do 29 i=1,legs
c
...
c
 ...
 37   format (1x,i4,3x,6f10.2,2x,4f10.3)
      avdt=avdt+b(j1)
      avdx=avdx+b(j1+1)
      avdy=avdy+b(j1+2)
      avdz=avdz+b(j1+3)
...
c
      do i=1,nmod
         write(16,71) i,nitt
71       format(1x,'Velocity model',i4,5X,'#',i2)
         k2=laysum(i)
         j11=j1+k2-1
         j22=j11+nplay(i)-1
         kj=1
...
c
      if(.not.single_turbo)then
         write(16,'(1x,i3,1x,i4,2i2.2,1x,2i2.2,f6.2,
...
c
      if(isingle.ne.0)then
         write(6,'(1x,i4,2i2.2,1x,2i2.2,f6.2,
     &              1x,f7.4,a1,1x,f8.4,a1,1x,
...
c
cek     write(7,'(i4,2i2,1x,2i2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,2f7.2)')
...
cek
      write(7,'(i4,2i2,1x,2i2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,2f7.2,
     &         4x,i3,5x,f5.2)')
     &         iyr(i),imo(i),iday(i),ihr(i),nin,sec,
     &         xlat,cns,xlon,cew,
...
cek   next line adjusted by EK 3.12.90
      read(cline,'(i4,1x,2i2,1x,2i2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,1x,
     &      f7.2,2x,f5.2)',err=9999,end=999)
...
c
   ...
   10 j1=j2+1
      j2=j1+5
      read(iunit,'(6(a7,a1,i1,2x,f6.2))',end=999)
...
c
   2  read(iunit,'(a)',end=999) cline
      if(cline.eq.' ') goto 2
c
      read(cline,'(i4,2i2,1x,2i2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,1x,f7.2,
     1      2x,f5.2)',
     &           err=9999,end=999)
     &           iyr,imo,iday,ihr,imin,origtime,
     &           xlat,cns,xlon,cew,depth,xmagni !  i1  =ifx(i)
...
c
      ...
      do j=1,maxobsperevent
         read(iunit,'(2x,a7,2x,a1,3x,i1,3x,f6.2)',end=99)
...
c     if NOBS=-1 ==> end of -file detected!
c
      implicit none
c
      integer maxobsperevent
...
c
     ...
     &                                        depth,xlat,xlon
      ...
      read(cline,'(a7,a1,a1,a1,i1,a1,5i2,f5.2,7x,
...
c
     ...
     &                 '' obtained:'')') nitt,ibackups
		 ...
		 write(*,10) davar1,xmsqrs1,sqrt(xmsqrs1)
		 write(98,'(i2,2x,3f12.6)') nitt,davar1,xmsqrs1,sqrt(xmsqrs1)
 10      format(' DATVAR=',f10.4,' mean sqrd residual= ',f10.4,
     &          '  RMS RESIDUAL= ',f10.4)
      endif
c
...
c
     ...
     &            rp,nrp,nrtn,jl,tkj,1,ttt,MLL,ster,direr,refrer,
     &            refler,DTDDrefl,DTDHrefl)
c
...
c      
 ...
 1301    format(/,' event =',i5,' station ',a7,' num. raypoints= ',i5)
      ...
      if(isingle.eq.0)then
         ...
         do i=1,legs
            if(abs(avres(i)).gt.1.0)then
               write(16,'('' Event# '',i5,'' >>> '',1x,i4,2i2.2,1x,2i2.2,
     &                    ''  AVRES ='',f6.2,'' NOBS ='',i3)')
...
3        format(1x,'Velocity readjustments:')
         write(16,27) k2,nitt
27       format(1x,'Velocity model',i4,5X,'#',i2)
      endif
      do 4 jjj=j1,j2
      k=k+1
      b(jjj)=b(jjj)/2.
...
c
      ...
      if(.not.single_turbo)then
      write(16,*)'WARNING:'
      write(16,'('' Iteration# '',i3,'' Event#'',i5,'' Res#'',i3,
     &   '' ='',f7.2,''; ABS > 2.0 ---> weight set to zero !'')')
     &   nitt,isingle,nobs,res(nobs,i)
      endif
      write(6,'('' Iteration# '',i3,'' Event#'',i5,'' Res#'',i3,
     &   '' ='',f7.2,''; ABS > 2.0 ---> weight set to zero !'')')
     &   nitt,isingle,nobs,res(nobs,i)
      if(.not.single_turbo)then
         write(16,*)'knobs(i)   = ',knobst
...
c
      ...
      if(.not.single_turbo) write(16,*)
	call FREEUNIT(iunit)
	open(iunit,file='velini.sta',status='unknown')
      do i=1,nsta
         if(nactualsta(i).gt.0)then
            if(.not.single_turbo)then
               if(nsp.eq.2) then
               write(16,'('' readings for station '',a7,'' : tot='',
     &                  i4,''  P:'',i4,''  S:'',i4)')
     &                  stn(i),nactualsta(i),nobsp(i),nobss(i)
	write(iunit,'(a7,2f12.4,i5)')stn(i),xla(i),xlo(i),ielev(i)	 
               else
               write(16,'('' readings for station '',a7,'' :'',i4)')
     &                  stn(i),nactualsta(i)
	write(iunit,'(a7,2f12.4,i5)')stn(i),xla(i),xlo(i),ielev(i)
               endif
            endif
     ...
     &           nstaeff
         write(16,*)
         write(16,'('' Total number of readings: '',i8)') nofreadings
         write(16,'('' Total number of P readings: '',i8)') nofreadp
         write(16,'('' Total number of S readings: '',i8)') nofreads
         write(16,*)
      endif
c
      RETURN
...
c    &               3x,i4,2i2.2,1x,2i2.2,1x,f5.2)
         write(16,51)
...
52       continue
     ...
     &                 del(j),j=1,knobs(i))
53       format(2(1x,a7,1x,a1,1x,i2,f7.3,2f6.2,4x))
2     continue
      write(16,*)
...
c
         ...
         if(bb(m).lt.-0.1)then
           iwarn=1
           write(16,'(5x,''WARNING: Station = '',a7,
     &          5x,''!!! Variance  bb('',i3,'') = '',f7.3,'' < 0 !!'')')
     &          stn(m),m,bb(m)
cek           write(6,'(5x,''WARNING: Station = '',a7,
...
cek         if(iwarn.eq.0)then
         write(16,'(1x,a7,3x,a1,i4,3f8.4,1x,2f8.4)') 
...
C 		write(*,*)'input the output file of mod:'
C 		outmodfile='veloutmod.txt'	  
         open(23,file=outmodfile,status='unknown')	  
         write(23,*)'Output model:'
		 write(*,*)'Output model:',outmodfile
c	 write(12,*)(nplay(m),m=1,nmod)
	 do m=1,nmod
            write(23,*)nplay(m)
	    do i=1,nplay(m)
	       write(23,'(f5.2,5x,f7.2,2x,f7.3,4x)')
...
c
      if(ze.lt.d(1)) then
	  write(*,*)'station ',xr,yr,zr
	  write(*,*)'event ',xe,ye,ze	  
	  stop'RAYPATH>>> earthquake ABOVE model!'
	  endif
      if(zr.lt.d(1)) then
	  write(*,*)'station ',xr,yr,zr
	  write(*,*)'event ',xe,ye,ze	  
	  stop'RAYPATH>>> station ABOVE model!'
	  endif
      depth=ze-zr
